...
# callbacks passthru
	def Init(self, info=None):
		mod.callbacks.onInit(info)
		return

	def Start(self, info = None):
		script = f"op('{self.ownerComp}').DoCallback('onStart', callbackInfo={info})"
		run(script)
	...
	def Exit(self, info=None):
		...
		script = f"op('{self.ownerComp}').DoCallback('onExit', {info})"
		run(script)
		return
		
	def Done(self, info=None):
		script = f"op('{self.ownerComp}').DoCallback('onDone', {info})"
		run(script)
		return
